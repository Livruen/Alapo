...
public class HybridNotCoveredTest extends ComparisonTest {
	...
	public void eStatusIllegal() {
		...
		assertEquals("Groﬂer Turm kann schlagen", EStatus.LEGAL, board.move("A1A6"));
	...
	public void evaluateTest() {
		...
		assertTrue("Vorteil Weiﬂ. Weiﬂ hat zwei, Schwarz hat nur einen", board.evaluate() > 0);
		
		board.move("B6C6");
		board.move("A6C6");
		
		assertEquals("Weiﬂ hat zwei, Schwarz hat nur einen.!",IBoard.WHITEWINS, board.evaluate(), 0.001);
		
		board.init("WTA1 STA6");
		board.move("A1A2");
		board.move("A6A2");
		
		assertEquals("Weiﬂ hat zwei, Schwarz hat nur einen.!",IBoard.BLACKWINS, board.evaluate(), 0.001);
	}
